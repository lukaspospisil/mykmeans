% prepare data
its = zeros(length(ns),length(Ts),6);
        ...
        for alg = 1:6
            ...
            for n_randidx = 1:length(Ns_rand)
                if computed(nidx,Tidx,n_randidx,alg) == 1
                    its_n_rand = [its_n_rand,out{nidx,Tidx,n_randidx,alg}.it_all];
                end
            end
            
            its(nidx,Tidx,alg) = mean(its_n_rand)/length(Ns_rand);
        end
    end
end
...
if and(length(ns) > 1, length(Ts) > 1)
    ...
    title('average number of (outer) iterations')
    
    mylegend = cell(numel(to_plot),1);
    for alg = 1:numel(to_plot)
        surf(ngrid,Tgrid,its(:,:,to_plot{alg}.alg)',...
    ...
    zlabel('number of iterations','Interpreter','latex')
