% prepare data
Ls = zeros(length(ns),length(Ts),6);
        ...
        for alg = 1:6
            ...
            for n_randidx = 1:length(Ns_rand)
                if computed(nidx,Tidx,n_randidx,alg) == 1
                    Ls_n_rand = [Ls_n_rand,out{nidx,Tidx,n_randidx,alg}.L];
                end
            end
            
            Ls(nidx,Tidx,alg) = mean(Ls_n_rand);
        end
    end
end
...
if and(length(ns) > 1, length(Ts) > 1)
    ...
    title('approximation quality (error)')
    
    mylegend = cell(numel(to_plot),1);
    for alg = 1:numel(to_plot)
        surf(ngrid,Tgrid,Ls(:,:,to_plot{alg}.alg)',...
